namespace Nummercial_finalProject.ViewModelController
{
    ...
    public class PersonViewModelController
    {
        ...
        // Change this to store enum values (list of Governments)
        public List<Governments> LocalGovernments { get; set; } = new();

        public string LGovermentsSerialized
        {
            get => string.Join(",", LocalGovernments.Select(g => g.ToString()));
            set => LocalGovernments = value.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                                            .Select(v => Enum.Parse<Governments>(v))
                                            .ToList();
        }
